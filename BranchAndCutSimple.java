...
//说明：分支回调只是人工干预，没有实际作用
//作演示 分支切割框架  用
public class BranchAndCutSimple {
	...
	//Branch callBack
	private static class BranchCB extends IloCplex.BranchCallback{
		private final IloNumVar[] used;
		private final IloNumVar[][] supply;
		
		public BranchCB(IloNumVar[] used,IloNumVar[][] supply)
		{
			this.used=used;
			this.supply=supply;
		}
		...
		public void main() throws IloException
		{
			if(isIntegerFeasible()) return;//整数可行 不处理
			
			IloNumVar bestVar=null;//选择要分支的变量
			double bestFrac=0.0;
			double bestValue=0.0;
			
			String selected="";//选择的变量
			
			//选择 |分数-整数|最大的那个变量
			
			//扫描used 
			for(IloNumVar var:used)
			{
			...
			{
				for(int j=0;j<supply[0].length;j++)
				{
					double val=getValue(supply[i][j]);
					double frac=Math.abs(val-Math.round(val));
					if(frac>bestFrac+eps)
					{
						bestFrac=frac;
			...
			if(bestVar!=null)
			{
				...
				System.out.println("分支一次："+selected);
				
				double down=Math.floor(bestValue);
				double up=Math.ceil(bestValue);
				
				//getObjValue() 估计节点的目标值，用来定 优先级
				makeBranch(bestVar,down,IloCplex.BranchDirection.Down,getObjValue());
				makeBranch(bestVar,up,IloCplex.BranchDirection.Up,getObjValue());
			}
	...
	public static void main(String[] args) {
		...
		try 
		{
			...
			IloNumVar[] used=new IloNumVar[nLocations];
			...
			for(int i=0;i<nClients;i++)
			{
				 for(int j=0;j<nLocations;j++)
				 {
					 supply[i][j]=model.boolVar("supply["+i+","+j+"]");
				 }
			}
			for(int j=0;j<nLocations;j++)
			{
				used[j]=model.boolVar("used["+j+"]");
			}
			//约束1
			for(int i=0;i<nClients;i++)
			{
			...
			//使用branch  callback
	         model.use(new BranchCB(used,supply));
